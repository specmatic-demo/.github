...
flowchart LR
  ...
  INVSYNC["Inventory Sync Service<br/>SUT: HTTP OpenAPI + Kafka/MQTT AsyncAPI"]
  ...
  RET -->|"MQTT/AsyncAPI"| NOTIF
  INVPROJ -->|"HTTP/OpenAPI"| CATA
```

## Bridge Services

- `inventory-sync-service`: Accepts inventory sync requests over HTTP, consumes inventory adjustment commands from Kafka (`inventory.adjustment.request`), publishes adjusted inventory state to MQTT (`inventory/adjusted`), and calls `catalog-service` over HTTP to keep inventory views aligned.
...
| `inventory-sync-service` | Bridge service that accepts sync requests over HTTP, consumes inventory adjustment events from Kafka, and republishes synchronized inventory updates over MQTT plus HTTP integration to catalog. | <ul><li>`contracts/services/inventory-sync-service/http/openapi.yaml` (OpenAPI, HTTP)</li><li>`contracts/services/inventory-sync-service/events/asyncapi.yaml` (AsyncAPI, Kafka + MQTT)</li></ul> | <ul><li>`contracts/services/catalog-service/http/openapi.yaml` (OpenAPI, HTTP)</li></ul> |
